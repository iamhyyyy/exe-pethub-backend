...
namespace EXE_PET_HUB.Infrastructure.Repositories
{
    public class MedicalRecordRepository : GenericRepository<MedicalRecord>, IMedicalRecordRepository
    {
        ...
        {
            return await _context.MedicalRecords.Where(r => r.PetId == petId).ToListAsync();
        }

        public async Task<List<MedicalRecord>> GetByAppointmentIdAsync(string appointmentId)
        {
            return await _context.MedicalRecords.Where(r => r.AppointmentId == appointmentId).ToListAsync();
        }
    }
}
